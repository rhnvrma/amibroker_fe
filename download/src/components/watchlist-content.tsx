...
interface WatchlistContextType {
  ...
  refreshItems: () => void; // Added this line
}
...
export function WatchlistProvider({ children }: { children: React.ReactNode }) {
  ...
  // --- START: Added refreshItems function ---
  const refreshItems = async () => {
    const refreshedItems = await window.electron.refreshItems();
    const refreshedItemKeys = new Set(refreshedItems.map(item => item.instrument_key));

    setWatchlists(prev => {
        const newWatchlists = prev.map(watchlist => ({
            ...watchlist,
            items: watchlist.items.filter(item => refreshedItemKeys.has(item.instrument_key))
        }));
        
        // You might want to also update the availableItems in the frontend
        // For now, we'll just filter the watchlists.

        return newWatchlists;
    });

    toast({ title: "Items refreshed and watchlists updated." });
  };
  // --- END: Added refreshItems function ---
